...
router.post('/', (req, res) => {
  ...
  res.redirect(
  `/confirmation.html?name=${encodeURIComponent(order.name)}&cakeOption=${encodeURIComponent(order.cakeOption)}&flavour=${encodeURIComponent(order.flavour)}&filling=${encodeURIComponent(order.filling)}`  import { Router } from 'express';
  import fs from 'fs';
  import path from 'path';
